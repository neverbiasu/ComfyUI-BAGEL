...
* constructing the packaged tokenizer and verifying its special-token IDs and
  vocabulary fingerprint against the converted metadata (no runtime download);
...
def _read_metadata(
    ...
    """Read + parse the converted metadata header, with actionable errors."""
    ...
    if metadata_json is None:
        ...
        if not os.path.isfile(sidecar_path):
            raise ValueError(
                f"{os.path.basename(path)} is not a converted BAGEL file "
                f"(missing embedded '{BAGEL_METADATA_KEY}' metadata and sidecar "
                f"{os.path.basename(sidecar_path)!r}). Run the converter or install "
                "the hash-bound ComfyUI repack sidecar next to the checkpoint."
            )
        try:
...
def discover_converted_bagel() -> Dict[str, str]:
    """Return ``{relative_display_name: safetensors_path}`` for converted BAGEL.
...
def _build_config(metadata: ConvertedBagelMetadata):
    """Construct the ``BagelConfig`` from embedded model configs (no download)."""
    model_configs = metadata.model_configs or {}
    if "llm_config.json" not in model_configs or "vit_config.json" not in model_configs:
        raise ValueError(
            f"Converted BAGEL {metadata.variant or '(unknown variant)'} is missing "
            f"embedded llm/vit configs; re-run the converter to embed them "
            f"(scripts/convert_bagel_model.py embeds configs from the source folder)."
        )
...
def _build_model(state_dict: Dict[str, torch.Tensor], config: BagelConfig) -> Bagel:
    ...
    if unexpected:
        # Unexpected keys mean the converted file does not match this model.
        raise KeyError(
            "Converted BAGEL contains unexpected weights not present in the "
            f"model (first 20): {sorted(unexpected)[:20]}. The converted file "
            "may target a different variant; verify the converter and source."
        )
    return model_obj


def _load_tokenizer(metadata: ConvertedBagelMetadata):
    ...
    expected = metadata.special_token_ids or {}
    if expected:
        mismatched = {
    ...
    if metadata.tokenizer_fingerprint:
...
def load_native_bagel(
    ...
    """Load a converted BAGEL safetensors file into a ``BAGEL_MODEL`` patcher.
    ...
    if not metadata.capabilities:
        raise ValueError(
            f"{os.path.basename(path)} advertises no supported capabilities."
        )
    ...
    print(f"[BAGEL] loaded converted model from {path} (variant={metadata.variant}, dtype={metadata.dtype})")
    return patcher
